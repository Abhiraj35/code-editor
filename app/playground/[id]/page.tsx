...
const MainPlaygroundPage = () => {
  ...
  const [fileSaveCounter, setFileSaveCounter] = useState(0);
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    if (failed.length === 0) {
      toast.success(`Saved ${succeeded.length} file(s)`);
    } else {
      const failedNames = failed
        .map((r: any) => `${r.reason.file.filename}.${r.reason.file.fileExtension}`)
        .join(", ");
      toast.error(
        `Saved ${succeeded.length}. Failed: ${failedNames}`
      );
    }
                      ...
                      <>
                        ...
                        <ResizablePanel defaultSize={50}>
                          <WebContainerPreview
                            ...
                            serverUrl={serverUrl || ""}
                            forceResetup={false}
                            fileSaveCounter={fileSaveCounter}
                          />
                        </ResizablePanel>
                      </>
                    )}
